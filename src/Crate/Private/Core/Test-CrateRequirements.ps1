...
.EXAMPLE
    Test-CrateRequirements
    Performs a complete system requirements check.

.EXAMPLE
    Test-CrateRequirements -AdminCheck
...
.NOTES
    ...
    License:     MIT License
#>
function Test-CrateRequirements {
    [CmdletBinding()]
    param(
        [Parameter()]
        [switch]$AdminCheck,
    ...
    process {
        ...
        if ($DiskSpaceCheck -or (-not $AdminCheck -and -not $ToolsCheck)) {
            ...
            $workspaceDrive = if ($global:CrateWorkspace) {
                Split-Path $global:CrateWorkspace -Qualifier
            }
            else {
                "C:"
            }
